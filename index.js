...
function addGamesToPage(games) {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    // add the class game-card to the element
...
gamesCard.innerHTML = GAMES_JSON.reduce((count, games) => {
  return count + 1;
}, 0);
/*************************************************************************************
...
const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${
  GAMES_JSON.length
} games. Currently, ${numUnfunded} game(s) remain(s) unfunded. We need your help to fund these amazing game(s).`;
